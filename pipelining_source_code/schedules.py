...
class _Action():
    ...
    def __repr__(self):  # noqa: D401 – keep simple string representation
        ...
        return ",".join(
            [
                ...
                str(self.computation_type),
                "" if self.microbatch_index is None else str(self.microbatch_index),
    ...
    def from_str(action_string: str):
        ...
        String should be formatted as [stage],[rank],[action type],[microbatch],[dest_rank]
            e.g. `0,0,F,0,1`, `1,,UNSHARD,,`, `3,,SEND_F,1,2`
        """
        action_string = action_string.strip()
        if match := _action_regex.match(action_string):
            ...
            return None
        raise RuntimeError(
            f"Invalid action string: {action_string}, should be formatted as [stage],[rank],[action type],[microbatch],[dest_rank] e.g. 0,0,F,0,1"
        )
