...
class MultiNodeAgentRunner:
    ...
    def run_on_nodes(
        ...
        #上线名称修改
        try:
            with fabric.Connection(host.ip, user = "root", port=host.port, connect_timeout=30) as conn:
            ...
            logger.warning(f"[Agent {agent_index}] SSH disconnected. Exception type: {type(e).__name__}")
            logger.warning(f"[Agent {agent_index}] Full exception: {e}")
            
            if hasattr(e, 'result'):
                result = e.result
                logger.error(f"[Agent {agent_index}] stdout:\n{result.stdout}")
                logger.error(f"[Agent {agent_index}] stderr:\n{result.stderr}")
