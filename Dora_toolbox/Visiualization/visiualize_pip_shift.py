...
def pip_ploting_shift(num_stages = 5, num_microbatches = 10,
            ...
            for s in range(num_stages):
                ...
                if m == 0 and s!=0: # no intra-dependency
                    if (s-1,m) in fp_schedule:
                        ...
                        if s%2 == 1:    # new added, for the percentage advance for communication...
                            start = fp_schedule[(s-1, m)][0]+start_percent*(fp_schedule[(s-1, m)][1]-fp_schedule[(s-1, m)][0])
                ...
                if m>0: #
                    ...
                    if s>0: # there is inter-dependency
                        if (s-1,m) in fp_schedule:
                            k = fp_schedule[(s-1, m)][1]
                            if s%2 == 1:
                                k = fp_schedule[(s-1, m)][0]+start_percent*(fp_schedule[(s-1, m)][1]-fp_schedule[(s-1, m)][0])
                ...
                if m>=0: #
                    ...
                    if s<num_stages-1: # there is inter-dependency                         
                        if (s+1,m) in bp_schedule:
                            k = bp_schedule[(s+1, m)][1]
                            if s%2 == 1:
                                k = bp_schedule[(s+1, m)][0]+start_percent*(bp_schedule[(s+1, m)][1]-bp_schedule[(s+1, m)][0])
                            start = max(start, k)
                        else:
                            continue
                    #then is intra-dependency
...
def pip_ploting_shift_real(RCPSP_tasks, TaskL = None,
                        ns=3, a=4, b=3, aa=5, 
    ...
    for i in range(fb_amount): 
        if_compute,if_first_part, nsteps, nmicrobatch, ntasks, if_back = sgf.partitioner_shift(i,s,b,a,aa)
        if if_compute == 0:
    ...
    for i in range(fb_amount, total_amount):
        nsteps, ntasks = sgf.partitioner_shift(i,s,b,a,aa)
        start, end = cj.RCPSP_sol_parser_shift(RCPSP_tasks,s, a,b,
                                         nsteps, nmicrobatch, 
                                         ntasks,
            ...
            for s in range(ns):
                ...
                if m == 0 and s!=0: # no intra-dependency
                    if (s-1,m,-1) in fp_schedule or (s-1,m,a-2) in fp_schedule: # interdepence
                        if s%2 == 1:
                            start = fp_schedule[(s-1, m, -1)][0]+start_percent*(fp_schedule[(s-1, m, -1)][1]-fp_schedule[(s-1, m, -1)][0]) # end time
                ...
                if m>0: #
                    ...
                    if s>0: # there is inter-dependency
                        if (s-1,m,-1) in fp_schedule or (s-1,m,a-2) in fp_schedule:
                            if s%2 == 1:
                                k = fp_schedule[(s-1, m, -1)][0]+start_percent*(fp_schedule[(s-1, m, -1)][1]-fp_schedule[(s-1, m, -1)][0])
                                start = max(start, k)
                            else:
                ...
                if m>=0: #
                    ...
                    if s<ns-1: # there is inter-dependency
                        ...
                        if (s+1,m,-1) in bp_schedule or (s+1,m,a-2) in bp_schedule:
                            if s%2 == 1:
                                k = bp_schedule[(s+1, m, -1)][0]+start_percent*(bp_schedule[(s+1, m, -1)][1]-bp_schedule[(s+1, m, -1)][0])
                                start = max(start, k)
                            else:
...
if __name__ == "__main__":
    ...
    #print(RCPSP_tasks)
    pip_ploting_shift(group_plan = [{'device':[0], 'layer':[0]}]*5)
    pip_ploting_shift_real(RCPSP_tasks,
                        ns=3, a=4, b=3, aa=5, 
