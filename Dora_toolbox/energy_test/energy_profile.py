...
def data_predeal(df):
    ...
    for gid, group in df.groupby("id"):
        cpu_energy_vals = group["cpu_energy"].values
        ...
        agg_results.append((gid, avg_throughput, robust_avg_cpu_energy))

    # Convert to DataFrame
    agg_df = pd.DataFrame(agg_results, columns=["id", "throughput", "cpu_energy"])
    return agg_df



# Load CSV
df = pd.read_csv("formal_summerize_ffinal.csv", skip_blank_lines=True)
...
unitbgE = np.mean(df['cpu_idle_energy'].values)/21.00
tokens = int(np.mean(df['total_new_tokens'].values))
print(unitbgE, tokens)

# Filter for Raspberry Pi and maxt=64
filtered = df[(df['name'] == 'amdcpu') & (df['maxt'] ==1)]
filtered = filtered.dropna(subset=['id', 'throughput', 'cpu_energy', 'cpu_idle_energy'])
...
for model_id in models:
    ...
    mask = (sub_df['cpu_energy'] - sub_df['cpu_idle_energy']) > -0.5e-6
    ax1.plot(
        agg_df['throughput'],
        (agg_df['cpu_energy'] - tokens/(agg_df['throughput'])*unitbgE),        
        marker='o',
