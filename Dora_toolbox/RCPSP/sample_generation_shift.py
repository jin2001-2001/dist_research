...
class TaskStatus_shift(TaskStatus_prime):
    def initialize(self):
...
def depedency_generate_shift(TaskL, s= 3, b=3, a=4, aa = 5,indexer = None, indexerg = None, scheduler = None):
    ...
    for i in range(steps-1): 
        ...
        for j in range(b):
            ...
            if(i %2 == 0): #computation step
                ...
                id2 = indexer(s,a,b,i+1,j)

                TaskL[id1].add_dependency(id2)
                TaskL[id2].add_precedence(id1)

                id1 = indexer(s,a,b,i+1,b+j)
...
def generatesm_shift(pfile = "scratch_shift.sm",s= 3, b=3, a=4,
                TProfile=[(10,20),(30,40),(50,60)], RProfile = [(15,17)]*1, UnitR = 7,
                gatheringTprofile= [20,0,20],gatheringRprofile= [20,0,20],
                aa = 5,percent = 0.6, indexer = indexer_shift, indexerg = gindexer_shift,
                d_generate = depedency_generate_shift, scheduler = OFOB_scheduler):
    total_amount = (s//2+1)*b*2*2+(s//2)*b*2*a   #one more *2 for two divided computation tasks...
    end_node_index = total_amount+s*aa
    #print(total_amount)
    TaskL = []
    for i in range(total_amount):
        #print(f"number: {i}")
        ptr = TaskStatus_shift(i, s,b,a , TProfile, RProfile, UnitR,
    ...
    steps = s
    Dep = []
    d_generate(TaskL, s, b, a, aa, indexer, indexerg, scheduler = scheduler)
    file_name = pfile
